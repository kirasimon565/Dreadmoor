...
class ScriptLoader {
  ...
  Future<void> importEpisode(String episodeId) async {
    ...
    if (manifest == null) { print("DreadmoorOS: No manifest for '$episodeId'."); return; }
    await _importIfNeeded(manifest);
  }

  Future<void> _importIfNeeded(EpisodeManifest episode) async {
    final existing = await (db.select(db.episodes)..where((e) => e.id.equals(episode.id))).getSingleOrNull();
    if (existing != null && existing.progress > 0) {
      print("DreadmoorOS: '${episode.id}' already imported, skipping."); return;
    }
    ...
    try {
      ...
      for (final path in episode.scenePaths) {
        String raw;
        try { raw = await rootBundle.loadString(path); }
        catch (_) { print("DreadmoorOS: Scene not found, skipping: $path"); continue; }
        final nodes = (jsonDecode(raw) as Map<String, dynamic>)['scenes'] as List<dynamic>;
        for (final node in nodes) { await _parseAndInsertNode(node as Map<String, dynamic>); count++; }
      }
      await db.into(db.episodes).insertOnConflictUpdate(
        EpisodesCompanion.insert(id: episode.id, isUnlocked: const drift.Value(true), progress: drift.Value(count)));
      print("DreadmoorOS: '${episode.id}' done — $count nodes.");
    } catch (e, st) { print("DreadmoorOS: Import failed for '${episode.id}': $e\n$st"); }
  }
  ...
  Future<void> _parseAndInsertNode(Map<String, dynamic> nodeData) async {
    ...
    for (final key in nodeData.keys) { if (!coreKeys.contains(key)) metadata[key] = nodeData[key]; }
    await db.into(db.storyNodes).insertOnConflictUpdate(
      StoryNodesCompanion.insert(
        id: nodeId, type: type, senderId: drift.Value(senderId),
